...
entriesRouter.put("/:id", loginRequired, async(req, res) => {
    ...
    if(!req.body.emotions){
        ...
        if(emotions.isArray()){ //checking that emotions is an array
            emotions.foreach(emotionString => {
                if(!(typeof emotionString == "string" || emotionString instanceof String)){
                    return res.status(400).json({error: "Emotions is not an array of strings"});
                }
            });
        } else {
            return res.status(400).json({error: "Emotions property in body is not an array"});
        }
    }
...
entriesRouter.post("/:id", loginRequired, async(req, res) => {
    ...
    if(!name || !isBinary){
        return res.status(400).json({error: "Missing habit name or isBinary property"});
    }
    const habit = {};
